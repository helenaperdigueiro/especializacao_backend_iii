...
func (h *productHandler) ConsumerPrice() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		for _, idStr := range idList {
			...
			if err != nil {
				ctx.AbortWithError(400, fmt.Errorf("id enviado nao é um inteiro"))
				return
			}
			idsInt = append(idsInt, id)
		}

		consumerPriceResponse, err := h.s.ConsumerPrice(idsInt)
		if err != nil {
			web.Failure(ctx, 409, err)
			return
		}
		web.Success(ctx, 200, consumerPriceResponse)
